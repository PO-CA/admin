...
import { adjustPocaCredit, completePocaShipping, createPocaShipping } from '@/query/api/logiPocaStore';
...
export default function CustomerDetail({
  ...
  useEffect(() => {
    if (usersData?.pocaDiscountRate !== undefined && usersData.pocaDiscountRate !== null) {
  ...
  useEffect(() => {
    ...
    setSelectedOrderIds((prev) =>
      prev.filter((orderId) =>
        pocaOrders.some((order) => order.id === orderId && order.status === 'PENDING'),
      ),
    );
  }, [pocaOrders]);
  ...
  const allPendingSelected =
    pendingOrderIds.length > 0 && pendingOrderIds.every((id) => selectedOrderIds.includes(id));
                ...
                <Stack spacing={2}>
                  ...
                  <FormControl fullWidth>
                    ...
                    <Select
                      ...
                      label="구분"
                      value={creditType}
                      onChange={(event) => setCreditType(event.target.value as PocaCreditType)}
                    >
            ...
            >
              ...
              <Stack direction="row" spacing={1} alignItems="center">
                ...
                <Button
                  ...
                  color="secondary"
                  disabled={
                    selectedOrderCount === 0 || !usersData?.id || createShippingMutation.isPending
                  }
                  ...
                  <TableBody>
                    {pocaOrders.map((order) => (
                      <TableRow key={order.id}>
                        ...
                        <TableCell>
                          <Stack spacing={0.3}>
                            <Typography fontWeight={600}>{order.eventName ?? '상품'}</Typography>
                            <Typography variant="caption" color="text.secondary">
                        ...
                        <TableCell align="right">{currency.format(order.totalPrice)}</TableCell>
                        <TableCell align="center">
                          <Chip size="small" label={order.status} color={order.status === 'SHIPPED' ? 'success' : order.status === 'SHIPPING' ? 'info' : 'warning'} />
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                ...
                <TableBody>
                  {pocaShippings.map((shipping) => (
                    <TableRow key={shipping.id}>
                      ...
                      <TableCell align="right">{shipping.totalOrderCount ?? 0}</TableCell>
                      <TableCell align="right">
                        {currency.format(shipping.totalProductPrice ?? 0)}
                      </TableCell>
                      <TableCell align="center">
                        <Chip size="small" label={shipping.shippingStatus} color={shipping.shippingStatus === 'COMPLETED' ? 'success' : 'info'} />
                      </TableCell>
                      <TableCell align="center">
                        <Button
                          variant="outlined"
                ...
                <TableBody>
                  {pocaCredits.map((entry) => (
                    <TableRow key={entry.id}>
                      ...
                      <TableCell align="right" sx={{ color: entry.changeAmount >= 0 ? 'success.main' : 'error.main' }}>
      ...
      >
        ...
        <DialogActions>
          ...
          <Button
            ...
            disabled={
              selectedOrderCount === 0 || createShippingMutation.isPending || !usersData?.id
            }
          >
